          ...
          cartdata.map((item) => {
            ...
            return (
              ...
              sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                ...
                <input onChange={(e) => e.target.value === '' || e.target.value ===
                  '0' ? null : updateQuantity(item._id, item.size, Number(e.target.value))}
                  className='border max-w-10 sm:max-w-20 px-1 sm:px-2 py-1 '
                  type="number" min={1} defaultValue={item.quantity} />
                <img onClick={() => updateQuantity(item._id, item.size, 0)}
                  className='w-6 sm:w-8 cursor-pointer mr- 4' src={assets.bin_icon} alt="" />
          ...
          <div className='w-full sm:w-[450px]'>
            ...
            <div className='w-full text-end'>
              <button onClick={() => navigate('/placeorder')}
                className='bg-black text-white px-8 py-3 text-sm my-8 cursor-pointer'>
