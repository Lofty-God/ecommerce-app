...
const Add = ({token}) => {
  ...
  const[subCategory, setSubcategory] = useState('English')
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-start w-full gap-3'>
        ...
        <div>
          ...
          <div className='flex gap-3'>
            <div onClick={()=>setSize(prev=>prev.includes('S')? prev.filter(item=>item !== 'S'): [...prev, 'S'])} >
              <p className={`${size.includes('S')? "bg-pink-300" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>S</p>
            </div>
            <div onClick={()=>setSize(prev=>prev.includes('M')? prev.filter(item => item !== 'M'): [...prev, 'M'])} >
              <p className={`${size.includes('M')? "bg-pink-300" : "bg-slate-200"} px-3 py-1 cursor-pointer`} >M</p>
            </div>
            <div onClick={()=>setSize(prev=>prev.includes('L')? prev.filter(item => item !== 'L') : [...prev, 'L'])} >
              <p className= {`${size.includes('L')? "bg-pink-300" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>L</p>
            </div>
            <div onClick={()=>setSize(prev=>prev.includes('XL')? prev.filter(item => item !== 'XL'):[...prev, 'XL'])} >
              <p className={`${size.includes('XL')? "bg-pink-300" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XL</p>
            </div>
            <div onClick={()=>setSize(prev=>prev.includes('XXL')? prev.filter(item => item !== 'XXL'):[...prev,'XXL'])} >
              <p className= {`${size.includes('XXL')? "bg-pink-300" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XXL</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <button className='bg-black text-white w-28 py-2 mt-3' type='submit' id='submit'>ADD</button>
